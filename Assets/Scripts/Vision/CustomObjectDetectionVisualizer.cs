...
public class CustomObjectDetectionVisualizer : MonoBehaviour
{
    ...
    private void HandleBatch(List<BoxData> batch)
    {
        ...
        _live.Clear();
            ...
            // Check instantiation of UDP listener
            if (udpListener == null) return;

            // Get the latest label from the UDP listener and normalize it for comparison
            string targetLabel = udpListener.GetLatestLabel().Trim().ToLower();
